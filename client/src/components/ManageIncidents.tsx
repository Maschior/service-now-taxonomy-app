...
import { Plus, X, AlertTriangle, Edit2, Trash2, Globe, Home, Check } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            <AlertTriangle className="text-indigo-600 w-8 h-8" />
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white m-0">Incidentes</h1>
              <p className="text-gray-500 dark:text-gray-400 mt-1">Gerencie os incidentes (chamados) para cada módulo.</p>
            </div>
          </div>
          <button 
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gray-50/50 dark:bg-gray-800/50">
            <div className="flex flex-col md:flex-row flex-wrap gap-2 w-full md:w-auto">
              <select
                ...
                className="w-full md:w-auto min-w-[150px] rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:text-white"
              >
          ...
          {loading ? (
            ...
            <div className="p-8 text-center text-gray-500">Nenhum incidente cadastrado.</div>
          ) : (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50 dark:bg-gray-900/50 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <th className="py-3 px-4 text-sm font-semibold text-gray-600 dark:text-gray-300">Módulo</th>
                ...
                <tbody>
                  {filteredItems.map(inc => {
                    ...
                    return (
                    <tr key={inc._id} className={`border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group ${selectedIds.includes(inc._id) ? "bg-indigo-50/50 dark:bg-indigo-900/20" : ""}`}>
                      ...
                      <td className="py-3 px-4 text-right">
                        {editingId === inc._id ? (
                          <div className="flex justify-end gap-1">
                            <button onClick={() => saveInlineEdit(inc)} className="p-1.5 text-green-600 hover:bg-green-50 rounded" title="Salvar">
                              <Check size={16} />
                            </button>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in-up">
            <div className="p-5 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <Plus className="text-indigo-500 w-5 h-5"/> Novo Incidente
              </h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nome do Incidente</label>
                <input
                  ref={nameInputRef}
                  autoFocus
                  type="text"
